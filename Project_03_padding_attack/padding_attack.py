...
class MD5(object):
    ...
    def fill_attack_text(self, original_len, message_append):
        total_len = (original_len + len(message_append) + 9) * 8  # calculate total length including the appending message
        padding_len = (448 - total_len % 512 + 512) % 512  # calculate padding length
        original_len = original_len << 3
        original_len = original_len & 0xffffffffffffffff
        original_len_bits = int2bin(original_len)
        padding = '1' + '0' * (padding_len - 1) + original_len_bits[::-1]  # new length appended
        return message_append + padding

# 省略MD5 object的其他部分

    def padding_attack(self, A, B, C, D, original_len):
        filled_message = self.fill_attack_text(original_len, self.message)
        self.message = filled_message  # update message with our new appended message
        self.fill_text()
        self.A = A
        self.B = B
        self.C = C
        self.D = D
        result = self.group_processing()
        return result
...
message_append = input("输入附加的字符串：")
original_message_len = len(message)
Attack = MD5(message_append)
attack_result = Attack.padding_attack(md5.A, md5.B, md5.C, md5.D, original_message_len)
print("利用长度扩展攻击得到的结果{}".format(attack_result))

message_all = message + message_append
all = MD5(message_all)
all.fill_text()
result_all = all.group_processing()
print("正常加密得到的结果{}".format(attack_result))
